...
asset URL 三种形式（与 SDK `validate_image_url` 一致，video/audio 同理）：

1. `https://...` / `http://...`（公网或后端可达的内网 URL）
2. `data:image/<fmt>;base64,<payload>` —— 仅限图片，且不超过单图大小上限
3. `asset://<asset-id>` —— 由 `POST /api/v1/assets` 上传后获得（**HOM-25 实现，本文档只占位**）
...
### 3.6 `DELETE /api/v1/tasks/{task_id}`
...
3. 上传 / 引用素材时，后端要校验 URL scheme：仅允许 `https://`、`http://`（限内网 / 白名单域）、`data:image/...`、`asset://...`。
...
- WebSocket / SSE 状态推送。
- 任务批量取消、批量重试。
- 自建对象存储 / 上传 endpoint：归 HOM-25 设计。
