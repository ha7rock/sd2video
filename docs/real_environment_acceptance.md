...
| HOM-23 | 前后端 API 契约 | ✅ `docs/api_contract.md` |
| HOM-24 | HTTP 后端服务（包 SDK） | ✅ 集成分支包含 `sd2video.server` |
| HOM-25 | 参考素材上传/URL 策略 | ✅ 契约与后端 payload shaping 已覆盖 URL 资源 |
| HOM-26 | 前端面板 → 后端 create task | ✅ 集成分支已接入 |
| HOM-27 | 状态轮询 / 结果预览 / 错误展示 | ✅ 集成分支已接入 |
| HOM-28 | 历史任务列表 | ✅ 集成分支已接入 |
| HOM-29 | 取消/删除 + 成本保护 | ✅ 集成分支已接入 |
| HOM-31 | mock 集成测试与 CI 门禁 | ✅ `python3 -m unittest discover -s tests` 默认不触发真实生成 |

本 runbook 应在包含 HOM-24 ～ HOM-29 的集成分支上执行。若这些能力缺失，**前端 → 后端 → Ark 的端到端闭环不存在**，跳到下面的 "Stage A：SDK-only" 也只能验证 SDK 与 Ark 的两段联通，不算完整验收。
...
5. 后端进程的启动入口是 `python -m sd2video.server`（或 README 指定的 `sd2video-server` 命令），并从环境变量读取上述配置。前端只通过后端 HTTP 调用 Ark，不能直接读取 `ARK_API_KEY`。
...
- 默认 mock 测试要全部 OK（live smoke 默认 skip）。
- 集成分支还会覆盖后端服务测试、前端契约测试和 mock-backend 端到端路径。
